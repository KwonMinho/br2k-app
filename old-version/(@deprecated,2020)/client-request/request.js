...
module.exports = {
  ...
  tx_log: function(_uri, data){
    if(data == null ) throw new TypeError('Data parameter is required')

    let option = {
      uri: _uri+'/tx',
      method: "GET",
      body: {tx_content: data},
      headers: {
        ServerState: "successful_tx",
  ...
  request:function(stateMode,reqOption,callback){
    if( stateMode == null) throw new TypeError('StateMode parameter is required')

    let sm = stateMode.toLowerCase()

    if( sm == 'stateful' || sm == 'stateless'){}
    else {
      throw new TypeError('StateMode parameter must be "stateful" or "stateless" ')
    }

    if(reqOption.headers == null) reqOption.headers = {ServerState:sm}
    else {
      reqOption.headers.ServerState = sm
    }
    req(reqOption,callback)
  },

  requestSync:function(stateMode,opts){
    return new Promise(function wrapped (resolve, reject) {
      ...
      if(opts.headers == null) opts.headers = {'Server-State':sm}
      else opts.headers = {'Server-State':sm}      
...
function req(requestOptions, callback) {
