...
module.exports = class Role {
  ...
  async _completionCheck() {
    ...
    if (res.length == 3) {
      let recordID = this._extractScvID(res);
      if(recordID == this.scvID){
        return false;
      }
    }
    return true;
   ...
   */
  setRLE(requestInfo) {
    this.routers.RLE.set(requestInfo.path, requestInfo.cb);
  }
   ...
   * @protected
   * @subject_of_use [leader, register, follower]
   * @param {object} rle: user request
   */
  async _storedState(rle) {
    const results = this.__findStateProcess(rle);
    const process = results.cb;
    const routingKey = results.key;

    const state = await process(rle);
   ...
   * @util
   * @protected
   * @param {string} res: request entry state (IF. res.length == 3)
   * @returns {string} scvID
   * */
  _extractScvID(res){
    let scvID = '';
  ...
  async _getRLE(index) {
    try {
      const result = await this.etcdClient.get(index);
      const rle = await JSON.parse(result);
      return rle;
    } catch (e) {
      return OPREATION.failed;
    }
   ...
   */
  __findStateProcess(rle) {
    const routingKey = this.__getRoutingKey(rle);
  ...
  async __isTransition() {
    ...
    // console.log(curEtcdLeader)
    // console.log('isLeaderStart: '+isLeaderStart);
    // console.log('isFollowerStart: '+isFollowerStart);
   ...
   *    maker: function to make RLE(Request Log Entry)
   * */
  _findRLEProcess(req){
    ...
    return {
      method: method,
      path: path,
      maker: this.routers.RLE.get(method.toLowerCase()+path)
    }
  }
