...
// } = require("../utils/config");
// let ETCD_CFG = null;
// if (process.argv[2] == 'etcd1') {
//   ETCD_CFG = {
//     instanceOption: { hosts: "localhost:2379" },
//   };
// } else if (process.argv[2] == 'etcd2') {
//   ETCD_CFG = {
//     instanceOption: { hosts: "localhost:2381" },
//   };  
// } else if (process.argv[2] == 'etcd3') {

//   ETCD_CFG = {
//     instanceOption: { hosts: "localhost:2383" },
//   };
// }
/////////////
...
class Leader extends Role {
  constructor(beforeRole, rcfg, Follower) {
  ...
  async _startLeaderRole() {
    ...
    if(this._isVersionUp()){
      this._startStateVersionUp();
      return;
    }
    this.checkRoleInstance = setInterval(this._checkRole, TIME.follwing_INTERVAL, this);
  ...
  async _processRequest(next, info) {
    ...
    /* bakcup entry */
    if(self._isVersionUp()){
      self._startStateVersionUp();
    }
   ...
   */
  _isVersionUp() {
    if (this.lpi >= this.versionUpSize) return true
    else return false
  }
  ...
  async _checkServicesHealth(retryCnt = 0) {
    ...
    for (let member of members) {
      try {
        serviceIDs.push({
          'service-id': member.ID,
          'location': member.clientURLs[0].replace(':2379','')
        });
  ...
  async _completionVersionUp() {
    try{
      ...
      const status = await this.etcdClient.maintenance.status();
      const revision = status.header.revision;
      await this.etcdClient.kv.compact({
        revision: revision,
        physical: true
      });
      const memberInfo = await this.etcdClient.cluster.memberList({linearizable: true});
      const members = memberInfo.members;
      for(let member of members){
        const tmp = new Etcd3({hosts: member.clientURLs});
        await tmp.maintenance.defragment();
        this.logger.log('info','leader','service-state-version up',`defragment <--ETCD-${member.clientURLs}`);
      }
      this.logger.log('info','leader','service-state-version up','completion to cleanup etcd space');
      this.logger.log('info','leader','service-state-version up','completion');
      await this._connectUpdateLSEvent();
    }catch(e){
      console.log(e);
      this._completionVersionUp();
    }
  }
  ...
  async _transition() {
    ...
    if (this._isRunningAction()) {
      ...
      this.root.roleObj = new this.genFollower(this, null);
    }
  }
