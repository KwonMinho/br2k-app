...
module.exports = class Role {
  ...
  async _checkRole(self) {
    try {      
      ...
      self._wait()
      self.logger.log('error', '', 'check-role', `error etcd-connection....retry to connection`);
    }
  }
  ...
  setRouting(isReplicate, requestInfo) {
    if (isReplicate) {
      ...
      this.routers.onlyOnce.set(requestInfo.path, requestInfo.cb);
    }
  }
  /**
   * @public
  ...
  async _isValidStateVer() {
    try {
      const curStateVer = await this.serviceRegistry.getLatestStateVersion();
      const curMyVer = await this.etcdClient.get(this.myStateVersionKey);

      this.stateVersion = curStateVer;
      this.myStateVersion = Number(curMyVer);

      /*check inconsistent state version(state version in etcd, state version in service registry)*/
      const curEtcdStateVer = await this.etcdClient.get(ETCD_KEY.stateVersion);
      if (this.stateVersion != Number(curEtcdStateVer)){
        await this.etcdClient.put(ETCD_KEY.stateVersion).value(this.stateVersion);
      }
  ...
  async __isTransition() {
    ...
    if (isLeaderStart) {
      clearInterval(this.checkRoleInstance);
    ...
    if (isFollowerStart) {
      clearInterval(this.checkRoleInstance);
      this.logger.log('info','leader','check role',"transition etcd state: FOLLOWER");
      this.curTransition = false;
      return true;
    }
    this.curTransition = false;
  ...
  __findReqProcess(req, reqType) {
    ...
    if (reqType == RES_TYPE.onlyOnce)
      return this.routers.onlyOnce.get(routingKey);
  }
