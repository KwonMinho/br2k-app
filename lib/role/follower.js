...
class Follower extends Role {
   ...
   */
  startService(self, root) {
    if (self.backupManager.isLoadNewState()) {
      setTimeout(self.startService, 2000, self, root);
    } else {
  ...
  async _transition(currentRole) {
    ...
    if (this._isRunnigAction()) {
      ...
      this.root.roleObj = new Leader(currentRole, null);
    }
  }
  ...
  async _waitVersionupWork() {
    self.logger.log('info','follower','state version up','the leader is upgrading the version of service state in BR2k service');
    clearInterval(this.loopingInstance);
    this.loopingInstance = null;
